...
// 分布：<0.36 近战(僵尸/尸壳)，<0.64 骷髅，<0.86 蜘蛛，其余苦力怕。
// 沙漠中近战槽由尸壳(sunImmune)替换僵尸，白天也不会被日晒烧掉（同 MC 1.12）。
export function hostileKindFor(biome: Biome, roll: number): MobKind {
  if (roll < 0.36) return biome === 'desert' || biome === 'badlands' ? 'husk' : 'zombie';
  if (roll < 0.64) return 'skeleton';
  if (roll < 0.86) return 'spider';
  return 'creeper';
}
