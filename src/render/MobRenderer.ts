...
function buildModel(kind: MobKind): Model {
  ...
  if (kind === 'pig') {
    ...
    // 脸=黑，画在前脸(+x)——与鸡/僵尸等一致（朝 -mob.yaw 旋转后正对前方）。
    // 之前画在 +z → 比其他生物偏 90°，脸朝侧面("脸的方向不对")。特征左右分布在 z、薄面在 x。
    const F = 0x0d0d0d;
    P(0.06, 0.14, 0.13, F, 0.22, lH + 1.18, 0.11); // 左眼
    P(0.06, 0.14, 0.13, F, 0.22, lH + 1.18, -0.11); // 右眼
    P(0.06, 0.16, 0.1, F, 0.22, lH + 1.06, 0); // 嘴-中竖
    P(0.06, 0.1, 0.1, F, 0.22, lH + 0.98, 0.1); // 嘴-左下
    P(0.06, 0.1, 0.1, F, 0.22, lH + 0.98, -0.1); // 嘴-右下(倒T 苦力怕脸)
